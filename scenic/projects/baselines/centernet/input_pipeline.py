...
# Feature description for TFRecords generated by write_tfrecord.py
custom_tfrecord_feature_description = {
    'image': tf.io.FixedLenFeature([], tf.string),
    'image/id': tf.io.FixedLenFeature([], tf.int64),
    'image/height': tf.io.FixedLenFeature([], tf.int64),
    'image/width': tf.io.FixedLenFeature([], tf.int64),
    'image/channels': tf.io.FixedLenFeature([], tf.int64),
    'image/dtype': tf.io.FixedLenFeature([], tf.string),
    'objects/count': tf.io.FixedLenFeature([], tf.int64),
    'objects/id': tf.io.VarLenFeature(tf.int64),
    'objects/area': tf.io.VarLenFeature(tf.int64),
    'objects/label': tf.io.VarLenFeature(tf.string),
    'objects/label_id': tf.io.VarLenFeature(tf.int64),
    'objects/is_crowd': tf.io.VarLenFeature(tf.int64),
    ...
    'image/pixel_min_val': tf.io.FixedLenFeature([1], tf.float32),
    'image/pixel_max_val': tf.io.FixedLenFeature([1], tf.float32),
}
...
def custom_tfrecord_decode_example(data):
  """Convert write_tfrecord.py TFRecord into tfds builder format.

  Output format matches what coco_decode_example produces so that
  decode_coco_detection_example can be used as decode_fn unchanged.
  """
  h = data['image/height']
  w = data['image/width']
  c = data['image/channels']
  raw_bytes = data['image']
  dtype_str = data['image/dtype']
  # Decode raw bytes with the dtype that was used when writing.
  # tf.io.decode_raw requires a static dtype, so we branch on dtype_str.
  image = tf.cond(
      tf.equal(dtype_str, b'uint8'),
      true_fn=lambda: tf.reshape(
          tf.cast(tf.io.decode_raw(raw_bytes, tf.uint8), tf.float32), [h, w, c]),
      false_fn=lambda: tf.cond(
          tf.equal(dtype_str, b'uint16'),
          true_fn=lambda: tf.reshape(
              tf.cast(tf.io.decode_raw(raw_bytes, tf.uint16), tf.float32),
              [h, w, c]),
          false_fn=lambda: tf.reshape(
              tf.io.decode_raw(raw_bytes, tf.float32), [h, w, c]),
      ),
  )

  ymin = tf.sparse.to_dense(data['objects/bbox/ymin'])
  xmin = tf.sparse.to_dense(data['objects/bbox/xmin'])
  ymax = tf.sparse.to_dense(data['objects/bbox/ymax'])
  xmax = tf.sparse.to_dense(data['objects/bbox/xmax'])
  # Stack as [ymin, xmin, ymax, xmax] to match decode_boxes expectation
  bbox = tf.stack([ymin, xmin, ymax, xmax], axis=-1)

  return {
      'image': image,
      'image/id': data['image/id'],
      'image/pixel_min_val': data['image/pixel_min_val'][0],
      'image/pixel_max_val': data['image/pixel_max_val'][0],
      'objects': {
          'bbox': bbox,
          'id': tf.sparse.to_dense(data['objects/id']),
          'area': tf.sparse.to_dense(data['objects/area']),
          'is_crowd': tf.cast(
              tf.sparse.to_dense(data['objects/is_crowd']), tf.bool),
          'label': tf.sparse.to_dense(data['objects/label_id']) - 1, # make label_id 0-based
      },
  }


def _is_custom_tfrecord(path: str) -> bool:
  """Return True if path looks like a TFRecord glob pattern."""
  return '.tfrecord' in path


def _count_tfrecord_examples(tfrecord_pattern: str) -> int:
  """Count examples in sharded TFRecord files matching a glob pattern."""
  shard_files = sorted(tf.io.gfile.glob(tfrecord_pattern))
  count = 0
  for f in shard_files:
    count += sum(1 for _ in tf.data.TFRecordDataset(f))
  logging.info('Counted %d examples in %s', count, tfrecord_pattern)
  return count


def load_split_from_custom_tfrecord(
    batch_size,
    train,
    preprocess_fn,
    decode_fn,
  ...
  """Load a split from sharded TFRecords generated by write_tfrecord.py.

  Args:
    batch_size: int; Batch size.
    train: bool; Whether to load train or validation split.
    preprocess_fn: function; Data augmentation / resize transform.
    decode_fn: function; Decodes example dict into {'inputs', 'label'}.
    tfrecord_pattern: str; Glob pattern for TFRecord files,
      e.g. '/data/train/*.tfrecord'. Train and validation splits are passed
      separately so each can point to a different location.
    num_classes: int; Number of object classes.
    cache: bool; Whether to cache the dataset.
    max_size: int; Padded image size.
    max_boxes: int; Maximum number of boxes per image.
    remove_crowd: bool; Remove crowd-annotated objects during training.
    shuffle_buffer_size: int; Shuffle buffer size.
    shuffle_seed: int; Seed for shuffling.
    num_channels: int; Number of image channels.
    compression_type: str; TFRecord compression ('', 'GZIP', or 'ZLIB').
  ...
  def parse_and_decode(serialized):
    ...
    # Only assert spatial dims; channel dim is handled dynamically in
    # decode_and_normalize so that both 3-ch RGB and 4-ch RAW records work.
  ...
  def decode_and_normalize(x, **kwargs):
    pmin = x.pop('image/pixel_min_val')
    pmax = x.pop('image/pixel_max_val')
    x = decode_fn(x, **kwargs)
    x['inputs'] = (x['inputs'] - pmin) / (pmax - pmin)
    x['inputs'] = tf.clip_by_value(x['inputs'], 0., 1.)
    # Dynamically convert 4-channel RAW -> 3-channel at runtime using tf.cond
    # so that both RGB (3-ch) and RAW (4-ch) TFRecords are handled correctly.
    if num_channels == 3:
      actual_c = tf.shape(x['inputs'])[-1]
      x['inputs'] = tf.cond(
          tf.equal(actual_c, 4),
          ...
          false_fn=lambda: x['inputs'],
      )
    return x

  ds = ds.map(
      lambda x: decode_and_normalize(
          x, remove_crowd=train and remove_crowd),
      num_parallel_calls=tf.data.experimental.AUTOTUNE)
  ...
  # Look for a COCO-format annotations.json next to the tfrecord shards.
  annotations_dir = os.path.dirname(shard_files[0]) if shard_files else ''
  ann_path = os.path.join(annotations_dir, 'annotations.json')
  test_annotation_path = ann_path if tf.io.gfile.exists(ann_path) else None
  return ds, {
...
def dataset_builder(*,
  ...
  train_is_custom = _is_custom_tfrecord(train_data_path)
  eval_is_custom = _is_custom_tfrecord(test_data_path)

  if train_is_custom:
    num_classes = dataset_configs.get('num_classes', 1)
    train_ds, _ = load_split_from_custom_tfrecord(
        batch_size, train=True,
        tfrecord_pattern=train_data_path,
        ...
        compression_type=dataset_configs.get('compression_type', ''),
  ...
  if eval_is_custom:
    num_classes = dataset_configs.get('num_classes', 1)
    eval_ds, ds_info = load_split_from_custom_tfrecord(
        eval_batch_size, train=False,
        tfrecord_pattern=test_data_path,
  ...
  num_train_examples = dataset_configs.get('num_train_examples') or 0
  num_eval_examples = dataset_configs.get('num_eval_examples') or 0
  if not num_train_examples:
    if train_is_custom:
      num_train_examples = _count_tfrecord_examples(train_data_path)
    else:
      num_train_examples = dataset_utils.get_num_examples(
          train_data_path, 'train')
  if not num_eval_examples:
    if eval_is_custom:
      num_eval_examples = _count_tfrecord_examples(test_data_path)
    else:
